...
let AboutReader = (() => {
  ...
  this._mm.addMessageListener("Reader:ResetZoom", this);*/
    ...
    let colorScheme = localStorage.getItem("_txtpage_color_scheme")||(matchMedia("(prefers-color-scheme: dark)").matches?"dark":"light");
  ...
  get viewId() {
    ...
    _setupFontSizeButtons() {
      ...
      let currentSize = parseInt(localStorage.getItem("_txtpage_font_size")) || 5;
      this._setFontSize(currentSize);
    ...
    _setupContentWidthButtons() {
      ...
      let currentContentWidth = parseInt(localStorage.getItem("_txtpage_content_width")) || 3;
    ...
    _setupLineHeightButtons() {
      ...
      let currentLineHeight = parseInt(localStorage.getItem("_txtpage_line_height")) || 4;
    ...
    _toggleToolbarFixedPosition(shouldBeFixed) {
      ...
      let contentWidth = this._doc.body.style.getPropertyValue(
        "--content-width"
      );
      if (shouldBeFixed) {
    ...
    _toolbarOverlapHandler() {
      ...
      if (this._toolbarContainerElement.classList.contains("dropdown-open")) {
        let toolbarBounds = this._toolbarElement.parentNode.getBoundingClientRect();
